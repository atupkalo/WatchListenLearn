...
export default function BePresent({ onHeaderDataReady, onAnswer }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await fetch('/data/1_to_be_present.json');
      ...
      onHeaderDataReady?.({
        ...
        description: lesson.lesson_desc || 'Practice to use verb To Be in Present tense',
      });
